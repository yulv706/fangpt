...
if __name__ == '__main__':
    ...
    # 动态确定采样数量，取生成数据和参考数据的最小值，最多10000
    sample_size = min(len(gen), len(data), 10000)
    print(f"使用采样大小: {sample_size}")
    
    data = data.sample(n = sample_size, random_state = 42).reset_index(drop = True)
    gen = gen.sample(n = sample_size, random_state = 42).reset_index(drop = True)
