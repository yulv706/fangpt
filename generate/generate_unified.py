...
def main():
    ...
    # 🔧 优先使用检测到的数据集名称，如果没有则使用命令行参数
    if 'data_name' in detected_config and detected_config['data_name']:
        data_name = detected_config['data_name']
        print(f"✅ 使用检测到的数据集: {data_name}")
    else:
        data_name = args.data_name
        print(f"🔧 使用命令行指定的数据集: {data_name}")
        ...
        for scaffold_cond in scaffold_loop:
            ...
            if molecules:
                ...
                results_df, validity, uniqueness, novelty = calculate_metrics(
                    molecules, results_df, data, data_name, args.batch_size, gen_iter
                )
